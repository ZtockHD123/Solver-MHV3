...
def graficar_datos(iteraciones, fitness, xpl, xpt, tiempo, mh, problem, corrida, binarizacion):
    ...
    output_dir = os.path.join(DIR_GRAFICOS, 'KP', str(binarizacion))
    os.makedirs(output_dir, exist_ok=True)

    # --- Gráfico de convergencia ---
    path_convergencia = os.path.join(output_dir, f'Convergence_{mh}_KP_{problem}_{corrida}_{binarizacion}.pdf')
    _, ax = plt.subplots()
    ax.plot(iteraciones, fitness, marker='o')
    ax.set_title(f'Convergence {mh}\n kp{problem} - Run {corrida} - ({binarizacion})')
    ...
    # --- Gráfico XPL vs XPT ---
    path_porcentaje = os.path.join(output_dir, f'Percentage_{mh}_KP_{problem}_{corrida}_{binarizacion}.pdf')
    ...
    path_tiempo = os.path.join(output_dir, f'Time_{mh}_KP_{problem}_{corrida}_{binarizacion}.pdf')
...
def graficar_boxplot_violin(instancia, binarizacion):
    ...
    direccion_datos = os.path.join(DIR_FITNESS, f'KP/fitness_KP_{instancia}_{binarizacion}.csv')
    ...
    # --- Boxplot ---
    output_dir_box = os.path.join(DIR_BOXPLOT, 'KP')
    os.makedirs(output_dir_box, exist_ok=True)
    file_path_box = os.path.join(output_dir_box, f'boxplot_fitness_KP_{instancia}_{binarizacion}.pdf')

    sns.boxplot(x='MH', y='FITNESS', data=datos, hue='MH', palette='Set2', legend=False)
    plt.title(f'Boxplot Fitness\n kp{instancia} - {binarizacion}')
    ...
    # --- Violinplot ---
    output_dir_violin = os.path.join(DIR_VIOLIN, 'KP')
    os.makedirs(output_dir_violin, exist_ok=True)
    file_path_violin = os.path.join(output_dir_violin, f'violinplot_fitness_KP_{instancia}_{binarizacion}.pdf')
...
def procesar_archivos(instancia, blob, archivo_fitness, bin_actual, mhs_instances):
    ...
    a una instancia específica del KP, utilizando una binarización concreta.

    Parámetros:
        instancia (str): Identificador del problema KP a analizar.
...
def graficar_mejores_resultados(instancia, mhs_instances, binarizacion):
    ...
    alcanzados por cada metaheurística en una instancia del KP.

    Parámetros:
        instancia (str): Nombre o identificador de la instancia KP.
    ...
    output_dir = os.path.join(DIR_BEST, 'KP')
    os.makedirs(output_dir, exist_ok=True)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'fitness_KP_{instancia}_{binarizacion}.pdf'))
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'time_KP_{instancia}_{binarizacion}.pdf'))
    plt.close()
...
def analizar_instancias():
    ...
    lista_instancias = ', '.join([f'"{func}"' for func in EXPERIMENTS["instancias"]["KP"]])
    ...
    for instancia in instancias:
        ...
        print(f"[INFO] Procesando instancia: kp{instancia_id}")
        blob = bd.obtenerArchivos(instancia_id)

        if not blob:
            print(f"[ADVERTENCIA] La instancia kp{instancia_id} no tiene experimentos asociados. Saltando...\n")
            continue
        ...
        for binarizacion in lista_bin:
            ...
            output_dir_resumen = os.path.join(DIR_RESUMEN, 'KP')
            output_dir_fitness = os.path.join(DIR_FITNESS, 'KP')
            os.makedirs(output_dir_resumen, exist_ok=True)
            os.makedirs(output_dir_fitness, exist_ok=True)

            # Inicializar archivos de salida
            archivoResumenFitness = open(os.path.join(output_dir_resumen, f'resumen_fitness_KP_{instancia_id}_{binarizacion}.csv'), 'w')
            archivoResumenTimes = open(os.path.join(output_dir_resumen, f'resumen_times_KP_{instancia_id}_{binarizacion}.csv'), 'w')
            archivoResumenPercentage = open(os.path.join(output_dir_resumen, f'resumen_percentage_KP_{instancia_id}_{binarizacion}.csv'), 'w')
            archivoFitness = open(os.path.join(output_dir_fitness, f'fitness_KP_{instancia_id}_{binarizacion}.csv'), 'w')
